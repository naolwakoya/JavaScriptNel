...
function handleSubmitButton () {
	...
	if(userText && userText != ''){
	   ...
	   var textDiv2 = document.getElementById("text-area");
	   ...
	   $.post("userText", userRequestJSON, function(data, status){
			...
			for (var i = 0; i < responseObj.newLine.length; i++){
				textDiv.innerHTML += "<p>" + responseObj.newLine[i]+ "</p>";
...
function findWord(x,y){
	var lowestPos = words[0];
	for(var i = 0; i < words.length; i++){
		if(words[i].x < lowestPos.x && words[i].y < lowestPos.y){
			lowest = words[i];
		}
	}
	return lowest;
...
function handleRefreshButton(){ 
	...
	console.log(words.sort(compareFunction));
	var sortedWords = words.sort(compareFunction);
	
	let textDiv = document.querySelector("#text-area");
	textDiv.innerHTML = "";

	for (var i = 0; i < words.length;i++){
			textDiv.innerHTML += sortedWords[i].word;
	}
...
function handleSaveAsButton(){
	...
	for(var i = 0; i < words.lenght; i++){
		allWords += words[i].word;
	}
	
	var blob = new Blob([allWords], {type: "text"});
	
	var songName = songName += $('#userTextField').val();
	songName += ".txt"; 
	
    var songsname = {name: songName, file: words}; 
    var pathName = "songs/";
    pathName += songsname;
    pathName += ".txt";
    var songsJSON = JSON.stringify(songsname);
    $.post(pathName, songsJSON, function(data,status){
        console.log("data: " + data);
        console.log("typeof: " + typeof data);
    });
    $('#userTextField').val('');
