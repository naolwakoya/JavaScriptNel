...
function handleMouseUp(e){
	...
	document.getElementById("canvas1").removeEventLisener("mousemove", handleMouseMove);
...
function handleKeyUp(e){
	...
	var x = new XMLHttpRequest();
       x.responseType = "json";
       x.open("POST", "example1.html");

		xhttp.send(jsonString);

		x.onload = function(){
			var robj = xhttp.response;
			wayPoints.push(robj);
			for (i in wayPoints) console.log(wayPoints[i]) ; //<-- use this if expecting 'application/json'
...
function handleSubmitButton () {
	...
	if(userText && userText != ''){
       ...
       xhttp.open("POST", "example1.html");

		xhttp.send(userRequestJSON);

		xhttp.onload = function(){
			var responseObj = xhttp.response;  //<-- use this if expecting 'application/json'
